...
>
        ...
        <div class="container flex items-center justify-between py-1">
            ...
            <div class="flex items-center gap-[16px]">
                ...
                @if (core()->getCurrentChannel()->currencies()->count() > 1)
                    <div x-data="{ currOpen: false }" class="relative">
                        <button
    ...
    <div class="container">
        <div class="flex items-center justify-between py-[12px] lg:py-[16px] gap-[16px]">
            ...
            {{-- Logo --}}
            <a href="{{ route('phonix.home') }}" class="shrink-0" aria-label="@lang('phonix::app.theme.name')">
            ...
            {{-- Desktop Navigation --}}
            <nav class="hidden lg:flex items-center gap-[24px]" aria-label="Main navigation">
                <a
            ...
            <form
                ...
                class="hidden md:flex flex-1 max-w-[400px] mx-[16px]"
                role="search"
            >
                <div class="relative w-full">
                    <input
                        ...
                        name="query"
                        value="{{ request('query') }}"
                        class="input-phoenix pe-[40px] ps-[40px] py-[10px] w-full"
                        placeholder="@lang('phonix::app.header.search.placeholder')"
                        aria-label="@lang('phonix::app.header.search.button')"
                        autocomplete="off"
                    />
                    <svg class="absolute start-[12px] top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-slate-400 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                    </svg>
                    <button
                        type="submit"
                        class="absolute end-[8px] top-1/2 -translate-y-1/2 p-[4px] text-slate-400 hover:text-phoenix-500 transition-colors"
                        aria-label="@lang('phonix::app.header.search.button')"
                    >
            ...
            <div class="flex items-center gap-[12px] lg:gap-[20px]">
                ...
                {{-- Account Dropdown --}}
                <div class="relative" @click.away="accOpen = false">
                    <button
                    ...
                    <div
                        ...
                        x-transition:leave-start="opacity-100 translate-y-0"
                        x-transition:leave-end="opacity-0 translate-y-1"
                        class="absolute end-0 top-full mt-[8px] w-[200px] bg-white dark:bg-dark-card rounded-lg shadow-xl border border-slate-100 dark:border-dark-border overflow-hidden z-50"
                        x-cloak
                    >
                        @guest('customer')
                            <a
                                href="{{ route('phonix.auth.login') }}"
                                class="flex items-center gap-[8px] px-[16px] py-[12px] text-sm text-slate-700 dark:text-slate-300 hover:bg-phoenix-50 dark:hover:bg-dark-surface transition-colors"
                            >
                                <svg class="w-[16px] h-[16px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        @auth('customer')
                            <div class="px-[16px] py-[10px] border-b border-slate-100 dark:border-dark-border bg-slate-50 dark:bg-dark-surface">
                            ...
                            <a href="{{ route('phonix.account.dashboard') }}" class="flex items-center gap-[8px] px-[16px] py-[10px] text-sm text-slate-700 dark:text-slate-300 hover:bg-phoenix-50 dark:hover:bg-dark-surface transition-colors">
                                <svg class="w-[15px] h-[15px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3h7.5v7.5h-7.5V3zM3.75 13.5h7.5V21h-7.5v-7.5zM13.5 3h7.5v7.5H13.5V3zM13.5 13.5h7.5V21H13.5v-7.5z"/></svg>
                                @lang('phonix::app.account.sidebar.dashboard')
                            </a>
                            <a href="{{ route('phonix.account.orders') }}" class="flex items-center gap-[8px] px-[16px] py-[10px] text-sm text-slate-700 dark:text-slate-300 hover:bg-phoenix-50 dark:hover:bg-dark-surface transition-colors">
                            ...
                            <a href="{{ route('phonix.account.wishlist') }}" class="flex items-center gap-[8px] px-[16px] py-[10px] text-sm text-slate-700 dark:text-slate-300 hover:bg-phoenix-50 dark:hover:bg-dark-surface transition-colors">
                                <svg class="w-[15px] h-[15px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>
                                @lang('phonix::app.header.account.wishlist')
                            </a>
                            <a href="{{ route('phonix.account.profile') }}" class="flex items-center gap-[8px] px-[16px] py-[10px] text-sm text-slate-700 dark:text-slate-300 hover:bg-phoenix-50 dark:hover:bg-dark-surface transition-colors">
                                <svg class="w-[15px] h-[15px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>
                                @lang('phonix::app.header.account.profile')
                            </a>
                            <div class="border-t border-slate-100 dark:border-dark-border">
                                <form action="{{ route('shop.customer.session.destroy') }}" method="POST" id="header-logout-form">
                                    @csrf
                                    @method('DELETE')
                                    <button
                                        type="submit"
                                        class="flex items-center gap-[8px] w-full px-[16px] py-[10px] text-sm text-coral hover:bg-red-50 dark:hover:bg-dark-surface transition-colors text-start"
                                    >
                                        <svg class="w-[15px] h-[15px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" /></svg>
            ...
            <form method="GET" action="{{ route('phonix.products.index') }}" role="search">
                <div class="relative">
                    <input
                        ...
                        name="query"
                        value="{{ request('query') }}"
                        class="input-phoenix ps-[40px] pe-[40px] py-[10px] w-full"
                        placeholder="@lang('phonix::app.header.search.placeholder')"
                        aria-label="@lang('phonix::app.header.search.button')"
                        autocomplete="off"
                    />
                    ...
                    <button type="submit" class="absolute end-[8px] top-1/2 -translate-y-1/2 p-[6px] text-phoenix-500 hover:text-phoenix-600 transition-colors" aria-label="@lang('phonix::app.header.search.button')">

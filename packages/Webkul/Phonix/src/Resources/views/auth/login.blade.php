...
<x-phonix::layouts.index :hasHeader="false" :hasFooter="false">
    <div class="min-h-screen flex items-center justify-center p-[16px] bg-gradient-to-br from-slate-50 via-phoenix-50/30 to-slate-50 dark:from-dark-bg dark:via-phoenix-950/20 dark:to-dark-bg">
        ...
        <div class="fixed inset-0 pointer-events-none" aria-hidden="true">
            <div class="absolute top-0 end-0 w-[400px] h-[400px] bg-phoenix-400/10 dark:bg-phoenix-400/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
            <div class="absolute bottom-0 start-0 w-[300px] h-[300px] bg-phoenix-600/10 dark:bg-phoenix-600/5 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2"></div>
        </div>
        ...
        <div class="w-full max-w-[420px] relative z-10" data-gsap="fade-up">
                ...
                >
                    ...
                    <div>
                        <label for="login-email" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-[6px]">
                            @lang('phonix::app.auth.login.email')
                        </label>
                        <input
                            type="email"
                            id="login-email"
                    ...
                    <div x-data="{ show: false }">
                        <label for="login-password" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-[6px]">
                            @lang('phonix::app.auth.login.password')
                        </label>
                        <div class="relative">
                            <input
                            ...
                            >
                                <svg x-show="!show" class="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                </svg>
                                <svg x-show="show" x-cloak class="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    ...
                    <div class="flex items-center justify-between">
                        <label class="flex items-center gap-[8px] cursor-pointer">
                            ...
                            <span class="text-sm text-slate-600 dark:text-slate-400">
                                @lang('phonix::app.auth.login.remember')
                            </span>
                        </label>
                        <a
                            href="{{ route('phonix.auth.forgot') }}"
                        ...
                        >
                            @lang('phonix::app.auth.login.forgot')
                        </a>
                    </div>

                    {{-- Submit --}}
                    <button type="submit" class="btn-phoenix w-full py-[14px]">
                        @lang('phonix::app.auth.login.submit')
                    </button>
                </form>
            ...
            <p class="text-center text-sm text-slate-500 dark:text-slate-400 mt-[24px]">
                @lang('phonix::app.auth.login.no_account')
                <a
                ...
                >
                    @lang('phonix::app.auth.login.register_link')
                </a>
            </p>
        </div>
    </div>
